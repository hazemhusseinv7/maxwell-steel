...
function CarouselNavigation({
  ...
  return (
    <div
      className={cn(
        "pointer-events-none absolute top-1/2 flex h-fit w-full -translate-y-1/2 justify-between px-2 lg:px-4",
        className,
      )}
    >
      <button
    ...
    >
      <div className="flex space-x-2">
        {Array.from({ length: itemsCount }, (_, i) => (
          <span
            key={i}
            aria-description={`Go to slide ${i + 1}`}
            onClick={() => setIndex(i)}
            className={cn(
...
function CarouselItem({ children, className }: CarouselItemProps) {
  return (
    <motion.div
      className={cn(
        "size-full min-w-0 shrink-0 grow-0 overflow-hidden",
        className,
      )}
    >
      {children}
